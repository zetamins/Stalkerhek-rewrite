...
impl FilterStore {
    ...
    pub fn strip_auto_prefix(title: &str) -> String {
        ...
        // Prefix must be 2-4 chars (country/language codes), no spaces.
        if let Some(pos) = title.find("| ") {
            let prefix = &title[..pos];
            if prefix.len() >= 2 && prefix.len() <= 4 && !prefix.contains(' ') {
        ...
        // Pattern 3: "PREFIX|Name" — no space around pipe (e.g. "UK|AMAZON UK EVENT")
        if let Some(pos) = title.find('|') {
            let prefix = &title[..pos];
